...
unsigned int bpsk_phase_state = 0;
unsigned int bpsk_delay = 31;
int bpsk_smoothing = 1;
int bpsk_step_delay = 333;

/*
void bpsk_setmode(int baud_rate){
...
// Some earlier versions of the phase reversal function reside here.
// The phase-reversal function, bpsk_phase_switch(), now resides in the PSK_STEPS.ino file,
// which can be generated using a matlab script.
/*
void bpsk_phase_switch2(){
bpsk_phase_state++;
...
void bpsk_phase_switch(){
    ...
    if(bpsk_smoothing){
		...
		/*
		if(bpsk_phase_state&1){
			for(unsigned int c = 0; c<0x7FF; c++){
				AD9834_SendWord(0xC000 | c);
			}
		
		}else{
			for(unsigned int c = 0x07FF; c>0; c--){
				AD9834_SendWord(0xC000 | c);
			}
		}
    	...
    	if(bpsk_phase_state&1){
    		digitalWrite(PSEL1, HIGH);
    	}else{
    		digitalWrite(PSEL1, LOW);
    	}
    }
}
*/
/*
void bpsk_tx_char(char c){
    uint16_t txword = bpsk_varicode[c];
    
    while(txword){
        uint8_t txbit = txword & 1;
        txword >>= 1;
    ...
    bpsk_phase_switch();delay(bpsk_delay);
    bpsk_phase_switch();delay(bpsk_delay);
}

void bpsk_tx_string( char * string){
  tx_on();
  char c;
...
// PSK Command, with baud rate
int parsePSK(String baud){
...
// PSKTERM Command, with baud rate
int pskTerminal(String baud){
	unsigned int baudrate = 0;
	char* endptr;
